...
interface SnapIconProps {
   ...
   *  tight content aspect ratio so the hand fills the box. Defaults to 18
   *  to match neighboring action icons. */
  size?: number;
  color: string;
}

/**
 * Line-art "snapping fingers" glyph used for the cast tip action — an
 * outline snapping hand with motion ticks.
 *
...
// The hand is tall and upright, so it fills the full box height. Call sites
// pass a `size` tuned for the old square glyph (a diagonal hand that read more
// compact). Scale height down a touch so it reads at a comparable optical size
// to the SF Symbol action icons beside it, while staying large enough to read.
const HEIGHT_RATIO = 0.92;

// The hand's visual mass sits low in its box, so in a center-aligned action row
// it reads as hanging below the SF Symbols. Nudge it up by a fraction of its
// height to sit better against them. Applied here (not at call sites) so every
// usage stays aligned.
const NUDGE_UP_RATIO = 0.08;

export function SnapIcon({ size = 18, color }: SnapIconProps) {
  const height = size * HEIGHT_RATIO;
  const width = (height * VIEWBOX.w) / VIEWBOX.h;
  ...
  const strokeWidth = (1.6 * VIEWBOX.h) / height;
  const common = {
    stroke: color,
    strokeWidth,
    fill: 'none' as const,
  ...
  return (
    <View style={{ transform: [{ translateY: -height * NUDGE_UP_RATIO }] }}>
    <Svg
      width={width}
      height={height}
