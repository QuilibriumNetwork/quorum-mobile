...
interface QuorumLogoIconProps {
  ...
  opacity?: number;
  /**
   * Fill colour. Defaults to the brand blue, which is also `accent[500]` on the
   * default accent — so existing call sites are pixel-identical.
   *
   * Pass `theme.colors.accent` where the mark should follow the user's chosen
   * accent (it is our own logo, so that reads as the app agreeing with itself).
   * Leave it alone where the mark identifies Quorum against something else and
   * has to stay recognisably blue — the cast like-row is the case in point.
   */
  color?: string;
}
 ...
 */
export function QuorumLogoIcon({
  size = 20,
  opacity = 1,
  color = brandColors.quorum,
}: QuorumLogoIconProps) {
  return (
