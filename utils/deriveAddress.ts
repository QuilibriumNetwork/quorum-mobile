 ...
 *
 * That weight is why the codebase already contains three hand-rolled copies of
 * this function (`hooks/chat/useChannelManagement`, `hooks/chat/useSpaceActions`,
 * `services/config/spaceSyncService`). Each is currently identical; nothing
 * makes them stay that way, and an address derivation that disagrees with
 * itself would mis-address messages rather than fail loudly. Those three should
 * come here too — deliberately not done in the same change as security work.
 *
