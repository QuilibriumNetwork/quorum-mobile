...
const EXCEPTIONS: Record<string, string> = {
  ...
  'components/Chat/types.ts':
    'Two unrelated things share this file. `directCastToDisplayMessage` and `castToDisplayMessage` (lines 571, 583, 590, 645, 670) read Farcaster `senderContext`/`actionTargetUserContext`/`cast.author` fields — a Farcaster identity, not a Quorum member, same class as `FarcasterCastCard.tsx` above. Separately, `toDisplayMessage` resolves a Quorum member through the PURE `resolveMemberName`/`formatResolvedName` from `@/utils/resolveMemberName`, by design: it is a plain data-transform function called from inside `.map()` in `DMChatArea.tsx`/`SpaceChatArea.tsx`, not a component, so it cannot call a hook — the same reason `utils/messagePreview.ts` and `services/notifications/logMentionOrReply.ts` stay on the pure API. What makes this SAFE is not that the QNS tier is inert — it fires routinely, because both call sites merge a real `primary_username` in from `useMembersWithPublicProfileFallback` and then run the map through `useVerifiedQnsNamesInMap` (`DMChatArea.tsx:199-200`, `SpaceChatArea.tsx:255-256`) BEFORE calling `toDisplayMessage`, so `resolveMemberName` only ever sees a map already stripped of unverified claims. `userName`, the field this produces, is read raw (not through `@/identity`) by at least `PinnedMessagesPanel.tsx:74` and `SpaceCallBubble.tsx:172`, in addition to `MessagesList.tsx`, which overrides it live via `@/identity`’s `useNameResolver()` rather than trusting it FOR A QUORUM SENDER — three consumers, not one. `MessagesList.tsx` deliberately does NOT override it for a FARCASTER sender (an `fc:`-prefixed cast author, or any sender in a list flagged `isFarcasterNamespace`): there the id is a fid, the resolver can only miss on it, and its truncating fallback returns a short id unchanged — so overriding would replace the name with the raw fid. That branch is the one place `userName` is read as the answer rather than as a starting point, and it is safe for the reason this whole Farcaster block exists: the value came from Farcaster’s own fields — `senderContext` for a DM sender, `cast.author` for a cast author — never from `toDisplayMessage`’s Quorum pipeline. Both producers additionally run those fields through `presentName`, so a Farcaster display name wearing the reserved `.q` suffix is dropped rather than rendered beside genuinely verified names in the identical style; `MessagesList.tsx` applies the same guard again at the render seam, so a future producer that forgets cannot reopen it. The verification pipeline at the two call sites is what keeps all of them safe; a future caller of `toDisplayMessage` that fed it a member map which skipped that pipeline would render a forged `.q`.',
